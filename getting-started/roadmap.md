...
- [x] **Role × Tool access matrix** *(historical — this table documents the original 4-tier role design. It was superseded by the 9-role Hub Role Matrix, managed live via the "Hub Role Matrix" SharePoint list and `main/shared/roleMatrix.js`/`scripts/setup-role-matrix.js` — treat those as the source of truth, not this table. A stale in-code reference to `hub.it`/`hub.standard` was the cause of the v3.6.1 Duo Management bug; full re-sync of this table to the current 9-role system is a separate, not-yet-scheduled cleanup)*
