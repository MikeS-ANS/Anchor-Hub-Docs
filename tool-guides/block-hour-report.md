...
- **Regenerate** — discards the current draft (including any edits) and asks Hatz.ai to write a fresh pair of narratives over the *same* stored hard numbers and work items (no new Autotask pull — those are facts about a past period that don't change between drafts).
...
The hard numbers (hours purchased/used/remaining, block expiration date) are never generated by the model — they're computed straight from Autotask's `ContractBlocks` data and handed to the prompt as already-computed, read-only facts the model is instructed to restate exactly, not recalculate.
...
- SQL schema migration: `functions/sql/phase14-block-hour-report-schema.sql` — one table, `bh_reports`, one row per client per month (`UNIQUE(companyId, periodMonth)`), mutated through a `pending → approved/skipped → sent` state machine rather than an append-only log, since the row itself doubles as the Send History entry.
