...
If any Autotask lookup fails, the tab shows an error with a Retry button instead of a table — a partial result would read as a real balance. If the Hub cannot reach its own server to find out whether a closed month has a snapshot, the month is read live from Autotask and an amber banner says so, because in that moment the Hub genuinely cannot tell "no snapshot" from "couldn't ask"; retry the load once the connection is back. If the Hub's settings service cannot be reached, On Hand and Picked still load but show an amber **Using default settings** banner, because the excluded-location list and lot-match window in force may not be the ones you saved.
...
Once a month-end snapshot exists for a month, the Summary tab shows that month's roll-forward — comparing this month's snapshot to last month's. For the still-open month, Summary explains that the roll-forward will appear once the month-end snapshot is taken on the 1st. For a closed month that has no snapshot, it says so and points you to Settings → Snapshot now to take a late one.
