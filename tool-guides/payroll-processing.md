...
The reason is the same logic the two single-shot reminders run on: the heads-up and the due-today message are each sent once per manager per period, never twice — so if the date they quoted turns out to be wrong, nothing would otherwise go back and correct it, and clearing those records is what lets both go out again against the new date. The overdue reminder doesn't need any of this: it already repeats on its own schedule and works out which date it's measuring against every time it fires, so it corrects itself at the next run without anything being reset. If a manager was told the deadline was the 23rd, and it turns out the 23rd is a holiday and gets moved to the 24th, they need a fresh heads-up for the 24th — not silence, because the Hub thinks it already warned them once. Without this, a manager would keep working to a date that no longer exists, having been told about it exactly once, and never corrected.
