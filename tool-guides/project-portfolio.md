...
* **Estimated revenue** — the dollar estimate on the project's contract, when there is one. The cell never shows $0 — a dash covers three situations instead: no contract attached to the project at all, a contract with no revenue estimate set, or a contract whose revenue is genuinely zero, and all three look identical in the grid. Hover the cell (now true of both Billing and Estimated revenue) or open the project to confirm whether there's a contract at all; the note doesn't yet say whether a "nothing to show" contract is blank or a real zero, so that distinction still means checking the contract itself in Autotask.
