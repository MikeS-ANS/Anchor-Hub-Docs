...
* A mapping screen for Duo, BitDefender, Liongard, and Lifecycle Manager X vendor accounts. All four are matched by name only today. As of this update they're also registered internally as mappable — a backend change with nothing to click on screen yet; the actual screen is Phase 3. Until it ships, an account whose vendor-side name doesn't match an Autotask company name still has to wait for that screen or a direct database entry — and for Duo and BitDefender specifically, until every unmapped account on the platform is mapped, that platform's **Unconfirmed** cells stay that way. There's no in-app way to clear one yet. Datto SaaS Protection isn't on this list and never will be: it's deliberately staying without a mapping entry point of its own, resolving instead through whatever Autotask company that client's Kaseya account already maps to.
